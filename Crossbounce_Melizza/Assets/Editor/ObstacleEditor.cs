...
public class ObstacleEditor : EditorWindow
{
    ...
    public static void ShowWindow()
    {
        ...
        instance.titleContent = new GUIContent("Object Creator");
    }

    private void OnGUI()
    {
        obstacleSpeed = EditorGUILayout.FloatField("Speed of Next Obstacle", obstacleSpeed, GUILayout.Width(200));
        if (obstacleSpeed > 15)
        {
            obstacleSpeed = 15;
        }
        if (GUILayout.Button("Create Obstacle", GUILayout.Width(150)))
        {
            ...
            if (obstacleGuids.Length > 0)
            {
                ...
                newWall.transform.GetChild(0).GetComponent<Collider2D>().sharedMaterial = physMat;
                newWall.transform.GetChild(0).GetComponent<ObstacleMovement>().obstacleSpeed = obstacleSpeed;
            }
        }
        ...
        if (GUILayout.Button("Create Crossbow", GUILayout.Width(150)))
        {
            //First fetch Guid assets
            string[] obstacleGuids = AssetDatabase.FindAssets("crossbowRoot");
            StringBuilder guiBuilder = new StringBuilder();
            ...
            if (obstacleGuids.Length > 0)
            {
                ...
                string assetPath = AssetDatabase.GUIDToAssetPath(trueObstacleGuids);
                //Get the object itself
                GameObject crossbowTemp = AssetDatabase.LoadAssetAtPath(assetPath, typeof(GameObject)) as GameObject;
                GameObject newCrossbow = GameObject.Instantiate(crossbowTemp);
                newCrossbow.name = crossbowTemp.name;
            }
        }
    }
}
